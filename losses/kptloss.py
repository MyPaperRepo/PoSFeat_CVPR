...
class DiskLoss(nn.Module):
    ...
    @ torch.no_grad()
    def constant_reward(self, inputs, outputs, coord1, coord2, reward_thr):
    ...
    @ torch.no_grad()
    def dynamic_reward(self, inputs, outputs, coord1, coord2, reward_thr):
        coord1_h = homogenize(coord1).transpose(1, 2) #bx3xm
        coord2_h = homogenize(coord2).transpose(1, 2) #bx3xn
        fmatrix = inputs['F1']
        fmatrix2 = inputs['F2']
    ...
    def forward(self, inputs, outputs, processed):
        ...
        # print('dense_p', dense_p)
        # print('dense_logp', dense_logp)
        # print('sample_plogp', sample_plogp)
        # print('min dense_p, logp, plogp', dense_p.min().item(), dense_logp.min().item(), sample_plogp.min().item())
        # print('max dense_p, logp, plogp', dense_p.max().item(), dense_logp.max().item(), sample_plogp.max().item())
